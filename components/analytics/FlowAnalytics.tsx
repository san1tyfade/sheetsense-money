...
import { XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ComposedChart, Line, Area, BarChart, Bar, Cell, LabelList, ReferenceLine, Label } from 'recharts';
import { LayoutGrid, Activity, Zap, Loader2, Terminal, Info, History } from 'lucide-react';
import { formatBaseCurrency } from '../../services/currencyService';
import { aggregateDimensions, aggregateComparativeTrend, getTemporalWindows, calculateTemporalVariance, calculateDimensionAverage } from '../../services/temporalService';
import { AnalyticsCard, StatHighlight, DrillBreadcrumbs, StandardTooltip } from './AnalyticsPrimitives';
...
export const FlowAnalytics: React.FC<FlowAnalyticsProps> = ({ 
  ...
  const handleBarClick = (data: any) => {
    ...
    if (!isTerminal) {
        ...
        // Normalization protocol for string comparisons
        const norm = (s: string) => (s || '').trim().toLowerCase();
        const yearMonth = nodeName; // "YYYY-MM"
        
        // 1. ATOMIC DRILL: Check Journal Buffer first for high-fidelity traceability
        const journalMatches = journalEntries.filter(j => {
            const dateStr = j.date || '';
            // Match Category, Sub-Category and Month Pattern with defensive normalization
        ...
        if (journalMatches.length > 0) {
            setInspector({
                ...
                transactions: journalMatches.map(j => ({
                    ...j,
                    type: activeType // Ensure type is forced for UI color logic
                })) as Transaction[]
            });
        } else {
            // 2. FALLBACK: Use Logical Summary from timeline if Journal is empty for this node
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 relative px-2">
        <div className="xl:col-span-7 bg-white dark:bg-slate-800/40 p-6 md:p-10 rounded-[3.5rem] border border-slate-200 dark:border-slate-700/50 flex flex-col h-[500px] md:h-[600px] shadow-sm relative group overflow-hidden transition-all duration-500">
          ...
          <div className="flex-1 w-full min-w-0 relative z-10">
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                ...
                >
                  ...
                  <YAxis 
                    ...
                    axisLine={false} 
                    tickLine={false} 
                    tickFormatter={formatDimensionName}
                    tick={{ fontSize: isMobile ? 9 : 12, fontWeight: 900, fill: '#64748b' }} 
            ...
            ) : <div className="h-full flex items-center justify-center text-slate-400 opacity-30 uppercase text-[10px] font-black italic">No Data In Window</div>}
          </div>
        </div>
            ...
            <div className="flex-1 w-full min-h-0 relative z-10">
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <ComposedChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{fontSize: 10, fill: '#94a3b8', fontWeight: 900}} tickFormatter={(v) => v ? new Date(v + '-02').toLocaleDateString(undefined, { month: 'short' }) : ''} />
                  <YAxis axisLine={false} tickLine={false} tick={{fontSize: 9, fill: '#94a3b8', fontWeight: 900}} tickFormatter={(v) => `$${v >= 1000 ? (v/1000).toFixed(0) + 'k' : v}`} width={50} />
