...
export const InformationView: React.FC = () => {
  ...
  const tabs = [ 
    { id:'tax', l:'Tax Matrix', i:ShieldCheck, desc: 'Contribution room & limits' }, 
    { id:'commitments', l:'Commitments', i:Receipt, desc: 'Recurring costs & debt' }, 
    { id:'accounts', l:'Institutions', i:Landmark, desc: 'Bank accounts & identity' } 
  ];
    ...
    <div className="space-y-12 animate-fade-in pb-24 px-2 md:px-0 tabular-nums">
      <header className="pt-2 pb-2">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
            ...
            <nav className="hidden md:flex items-center gap-3 text-base font-black tracking-tight">
                {tabs.map((t, idx) => (
                  <React.Fragment key={t.id}>
                    <button onClick={() => setActiveTab(t.id as any)} className={`transition-all duration-300 hover:text-slate-900 dark:hover:text-white ${activeTab === t.id ? 'text-slate-900 dark:text-white scale-105 underline decoration-blue-500 decoration-2 underline-offset-8' : 'text-slate-400'}`}>
                      {t.l.split(' ')[0]}
                    </button>
                    {idx < tabs.length - 1 && <span className="text-slate-200 dark:text-slate-700 font-light text-lg">/</span>}
                  </React.Fragment>
                ))}
            </nav>
        </div>
        ...
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-slate-900 rounded-t-[3rem] p-8 space-y-8 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-4">{tabs.map(t => (
                <button key={t.id} onClick={() => { setActiveTab(t.id as any); setIsMenuOpen(false); }} className={`w-full flex items-center justify-between p-6 rounded-[2rem] border-2 transition-all ${activeTab === t.id ? 'bg-blue-500/10 border-blue-500 text-blue-600 dark:text-blue-400' : 'bg-slate-50 dark:bg-slate-800 border-transparent text-slate-600 dark:text-slate-300'}`}>
                  <div className="flex items-center gap-5 text-left"><div className={`p-3 rounded-2xl ${activeTab === t.id ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'}`}><t.i size={20} /></div><div><p className="font-black uppercase text-xs tracking-widest">{t.l}</p><p className="text-[10px] opacity-60 font-bold mt-0.5">{t.desc}</p></div></div><ChevronRight size={18} className={activeTab === t.id ? 'opacity-100' : 'opacity-20'} /></button>))}
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="mt-20 pt-10 border-t border-slate-200 dark:border-slate-800 flex justify-center pb-12 opacity-50">
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">
             <div className="p-1 rounded bg-blue-500/10 text-blue-500 shrink-0"><Terminal size={12} /></div> Authorized Registry Instance v2.8.0
          </div>
      </footer>
    </div>
  );
