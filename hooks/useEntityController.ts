 ...
 */
export function useEntityController<T extends { id: string; rowIndex?: number; isDirty?: boolean }>(
    pool: T[],
    ...
    const add = useCallback(async (item: T) => {
        const nextItem = { ...item, id: item.id || crypto.randomUUID(), isDirty: true };
    ...
    const bulkEdit = useCallback(async (ids: Set<string>, updates: Partial<T>) => {
        setPool(prev => prev.map(item => 
            ids.has(item.id) ? { ...item, ...updates, isDirty: true } : item
        ));
