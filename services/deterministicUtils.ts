...
import { parseCSVLine, isSafeKey, normalizeTicker, detectTickerCurrency, MONTH_NAMES } from "./parsers/parserUtils";
...
const findHeaderIndex = (lines: string[][], dataType: string): number => {
    ...
    for (let i = 0; i < Math.min(lines.length, 50); i++) {
        const row = lines[i].map(v => v.toLowerCase().trim());
        const hitCount = allKeywords.filter(k => row.some(cell => cell.includes(k.toLowerCase()))).length;
        // If we find at least 2 matching columns, we've likely found the header
