...
export const cleanMerchantDescription = (desc: string): string => {
  ...
  return desc
    ...
    .replace(/#\s?\d+/g, '')
    // 2. Remove long numeric sequences (likely store IDs or sequence codes)
    .replace(/[A-Z0-9]{8,}/g, '')
    // 3. Remove common location suffixes and street identifiers
    .replace(/\s(STREET|AVE|AVENUE|ROAD|RD|BOULEVARD|BLVD|NORTH|SOUTH|EAST|WEST|NORTHHILL|SOUTHHILL)\b/g, '')
