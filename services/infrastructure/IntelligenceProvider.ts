...
export const cleanMerchantDescription = (desc: string): string => {
  ...
  return desc
    ...
    .replace(/#\s?\d+/g, '')
    // 2. Remove long numeric/alphanumeric sequences (must contain at least one digit to avoid stripping words like MERCHANT)
    .replace(/\b(?=\w*\d)[A-Z0-9]{7,}\b/g, '')
    // 3. Remove common location suffixes and street identifiers
    .replace(/\s(STREET|AVE|AVENUE|ROAD|RD|BOULEVARD|BLVD|NORTH|SOUTH|EAST|WEST|NORTHHILL|SOUTHHILL|BENTALL)\b/g, '')
...
export const intelligentCategorize = async (
  ...
  try {
    ...
    throw new AppError(IEP.INT.MODEL_TIMEOUT, `Neural Categorization Fault: ${error.message}`, 'RECOVERABLE', error);
  }
};
